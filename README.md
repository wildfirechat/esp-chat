...
| 页 | 文件 | 内容 |
|---|---|---|
| **会话** | [main/ui_convs.c](main/ui_convs.c) | 会话列表，最近的在上面。每行「名字 + 时间」「最后一句 + 未读角标」，群会话的最后一句带发言人。点一行进聊天页 |
| **聊天** | [main/ui_chat.c](main/ui_chat.c) | 消息气泡，自己的靠右（蓝），对方靠左；群里连续同一个人只在第一条上面写名字；群通知居中显示。进入即已读。底部一条「输入消息…」，单聊右边还有 📞 |
...
例子是两个类型：`1001` 普通自定义消息（带标签的气泡）和 `1002` 自定义通知（居中一行）。
